...
function Requirement({value}) {
    return (
        <>
            { value ?
                <>                    
                    <Typography component="p">{value.responsibility ? value.responsibility : ''}</Typography>
                    <Typography component="p">{value.requirement ? value.requirement : ''}</Typography>
                </> : null
            }
        </>
    );
...
function Address({value}) {
    return (
        <>
            { value ?
                <div>{ value.city ? value.city + ', ' + value.street + ' ' + 
...
function Salary({value}) {
    ...
    return (
        <Typography variant="h5" color="primary">{ result }</Typography>
    );
}
...
function Logo({value}) {
    return (
        <>
            { value ? <img src={value['90']}></img> : null }
        </>
    );
}
...
function Published({value}) {
    ...
    return (
        <Typography align='right' color='textSecondary'>{new Date(value).toLocaleString('ru', options)}</Typography>
    );
}
