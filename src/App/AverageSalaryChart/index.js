...
function getNormalizeSalaryArr(data) {
    ...
    for (let i = 0; i < data.length; i++) { 
        let val = data[i].salary ? (data[i].salary.from ? data[i].salary.from : (data[i].salary.to ? data[i].salary.to : 0)) : 0;
...
function AverageSalaryChart({data}) {
    ...
    return (
        <div style={{padding: '8px'}}>
            <Card>
                <CardContent>
                    <Typography variant="subtitle2">Средняя зарплата</Typography>
                    <Typography variant="h6">{average} руб.</Typography>
                    <Typography paragraph />
                    <Typography variant="subtitle2">Медианная зарплата</Typography>
